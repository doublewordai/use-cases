"""CLI for running model evaluations via batch API."""
...
@click.group()
def cli():
    """Model evaluation using Doubleword Batch API.

    Run large-scale evaluations affordably against GSM8K benchmark.
    """
    pass
...
@click.option(
    ...
    "-n",
    default=100,
    help="Number of questions to include (default: 100)",
)
@click.option(
    "--output",
    "-o",
    default="data/gsm8k_sample.json",
    help="Output file path",
)
def prepare(limit: int, output: str):
    """Prepare evaluation dataset (download GSM8K sample)."""
    click.echo(f"Loading GSM8K dataset (limit: {limit})...")
        ...
        def process_request(req):
            ...
            try:
                ...
                if provider_name == "openai":
                    params["max_completion_tokens"] = req.get("max_tokens", 1024)
                else:
                    params["max_tokens"] = req.get("max_tokens", 1024)
...
def score(results: str, dataset: str | None, output: str):
    ...
    if dataset is None:
        ...
        if dataset is None:
            dataset = "data/gsm8k_sample.json"
    ...
    if metrics["by_category"]:
        ...
        for cat, scores in metrics["by_category"].items():
            click.echo(f"  {cat}: {scores['accuracy']:.1%} ({scores['correct']}/{scores['total']})")
