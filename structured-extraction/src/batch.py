"""Batch and real-time API utilities."""
...
def stream_results(results_path: Path) -> Iterator[dict]:
    """Stream results from JSONL file."""
    with open(results_path) as f:
        for line in f:
            if line.strip():
...
def get_openai_client() -> AsyncOpenAI:
    """Create async OpenAI client for real-time API."""
...
async def run_realtime_extraction(
    ...
    """Run extractions via real-time API with async concurrency."""
    ...
    async def process_one(req: dict) -> dict:
        ...
        async with semaphore:
            ...
            if progress_callback:
                progress_callback(completed, len(requests_data))
            return result
