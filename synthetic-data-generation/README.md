...
Generating high-quality synthetic training data has traditionally required either expensive human annotation or costly API calls that make large-scale generation impractical. The other constraint is time: a three-stage pipeline (scenario generation, conversation generation, quality filtering) on the 24-hour turnaround typical of batch APIs would take three days minimum. On Doubleword's faster batch tier, our run completed in about three hours.
...
Option 3 is increasingly popular, but it has two bottlenecks: cost and latency. At realtime pricing, generating 10,000 samples costs over \$100. And with a multi-stage pipeline, you're waiting for each batch to complete before submitting the next. A 24-hour batch turnaround means three days of wall-clock time just for the batches, plus your own iteration cycles.

Doubleword's fast batch turnaround changes both equations. Cost drops by 97%, and a three-stage pipeline completes in hours rather than days. This shifts the approach from "generate the minimum viable dataset" to "generate abundantly and curate aggressively," which produces better fine-tuned models.
...
|----------|-------|-----|------------|-------------|------------|
| Doubleword (batch) | Qwen 30B | 1382 | \$0.07/MTok | \$0.30/MTok | **\$3.21** |
| Doubleword (batch) | Qwen 235B | 1423 | \$0.15/MTok | \$0.55/MTok | **\$6.13** |
